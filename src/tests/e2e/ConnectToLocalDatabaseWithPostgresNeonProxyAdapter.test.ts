...
describe("ConnectToLocalDatabaseWithPostgresNeonProxyAdapter", () => {
  test("should connect to local database and run queries", async () => {
    ...
    await adapter.db.schema.dropTable("people").execute();
    const tableExistsAfterDrop = await doesTableExist(adapter, "people");
    expect(
